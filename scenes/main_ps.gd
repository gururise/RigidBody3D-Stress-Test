...
func spawn_cubes() -> void:
	...
	for i in range(num_cubes):
		...
		cube_array.append(InnerCube.new(PhysicsServer.body_create(PhysicsServer.BODY_MODE_RIGID, true),i))
		var transform = Transform()
		PhysicsServer.body_add_shape(cube_array[i].rID,box)
		PhysicsServer.body_set_param(cube_array[i].rID,PhysicsServer.BODY_PARAM_MASS, 1.0)
		PhysicsServer.body_set_param(cube_array[i].rID,PhysicsServer.BODY_PARAM_GRAVITY_SCALE, 1.0)
		PhysicsServer.body_set_space(cube_array[i].rID,get_world().space)
		PhysicsServer.body_set_state(cube_array[i].rID,PhysicsServer.BODY_STATE_CAN_SLEEP,true)
		PhysicsServer.body_set_state(cube_array[i].rID,PhysicsServer.BODY_STATE_SLEEPING,true)
		...
		PhysicsServer.body_set_state(cube_array[i].rID,PhysicsServer.BODY_STATE_TRANSFORM,transform)
